...
void ScannerController::ScanStep(){
    ...
    // in some cases scanner begins to scan without entering into ScannerController::buttonPress,
    // so in those cases we should emit started() with first entering into ScannerController::ScanStep
    if (!m_started)
    {
        m_started = true;
        emit scanStarted();
    ...
    while (m.isNull() && n<10){
        ...
        emit progressUpdated(n * ProgressMultiplier);
    }
