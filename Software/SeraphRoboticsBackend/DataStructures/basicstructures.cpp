...
QDomNode nodeFromDate(QDateTime date){
    ...
    day.appendChild(d.createTextNode(QString::number(date.date().day())));
    node.appendChild(day);

    QDomElement month = d.createElement("month");
    month.appendChild(d.createTextNode(QString::number(date.date().month())));
    node.appendChild(month);

    QDomElement year = d.createElement("year");
    year.appendChild(d.createTextNode(QString::number(date.date().year())));
...
QDateTime dateFromNode(QDomNode node){
    ...
    if(!("date"==node.nodeName().toLower())){return QDateTime();}
    ...
    QDateTime d(QDate(year,month,day));
    return d;
}
